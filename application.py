...
@app.route("/")
def index():
    shirts = db.execute("SELECT * FROM Store ORDER BY onSalePrice")
    ...
    if 'user' in session:
        ...
        shirts = db.execute("SELECT * FROM Store ORDER BY onSalePrice ASC")
        shirtsLen = len(shirts)
...
def buy():
    ...
    if session:
        ...
        goods = db.execute("SELECT * FROM Store WHERE id = :id", id=id)
        ...
        shirts = db.execute("SELECT * FROM Store ORDER BY samplename ASC")
        shirtsLen = len(shirts)
...
def filter():
    if request.args.get('typeClothes'):
        ...
        shirts = db.execute("SELECT * FROM Store WHERE typeClothes = :query ORDER BY samplename ASC", query=query )
    if request.args.get('sale'):
        query = request.args.get('sale')
        shirts = db.execute("SELECT * FROM Store WHERE onSale = :query ORDER BY samplename ASC", query=query)
    if request.args.get('id'):
        query = int(request.args.get('id'))
        shirts = db.execute("SELECT * FROM Store WHERE id = :query ORDER BY samplename ASC", query=query)
    if request.args.get('kind'):
        query = request.args.get('kind')
        shirts = db.execute("SELECT * FROM Store WHERE kind = :query ORDER BY samplename ASC", query=query)
